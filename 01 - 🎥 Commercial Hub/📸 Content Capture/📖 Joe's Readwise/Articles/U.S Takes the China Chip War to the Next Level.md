# U.S Takes the China Chip War to the Next Level - Will Soon Stop Chinese Companies From Using American Clouds for AI Training

![rw-book-cover](https://cdn.mos.cms.futurecdn.net/sMWQJWfmn8kjuwnaMWdJHF-1200-80.jpg)

## Metadata
- Author: [[Anton Shilov]]
- Full Title: U.S Takes the China Chip War to the Next Level - Will Soon Stop Chinese Companies From Using American Clouds for AI Training
- Category: #articles
- Summary: The U.S. government is proposing a regulation that would prevent foreign entities, especially those from China, from using U.S. cloud computing for AI model training. The goal is to safeguard national security and maintain U.S. technological superiority. Chinese entities would still be able to access cloud services deployed in Europe and the Middle East. The proposed regulation, called 'Know Your Customer,' would require U.S. cloud companies to rigorously identify their foreign users and prevent entities from countries like China from accessing U.S. cloud resources for AI development. The initiative is part of a broader strategy to ensure that U.S. cloud platforms are not exploited for potentially hostile AI development.
- URL: https://news.bensbites.co/posts/16257-us-will-soon-stop-chinese-companies-from-using-american-clouds-for-ai-training/out

## Highlights
- The U.S. government is introducing a proposal to prevent foreign entities, particularly from China, from using U.S. [cloud computing](https://www.tomshardware.com/tag/cloud-computing) for AI model training, the U.S. Commerce Secretary Gina Raimondo announced this week, reports [Reuters](https://www.reuters.com/technology/us-propose-know-your-customer-requirements-cloud-computing-companies-2024-01-26/). ([View Highlight](https://read.readwise.io/read/01hn6e318esavqfzkjhhyyhsq7))
- Chinese entities can still access services deployed in Europe and the Middle East. ([View Highlight](https://read.readwise.io/read/01hn6e3d0rc32jyse2mndak0ds))
- We can’t have non-state actors or China or folks who we don’t want accessing our cloud to train their models,” Raimondo told *Reuters.* “We use export controls on chips,” she noted. “Those chips are in American cloud data centers so we also have to think about closing down that avenue for potential malicious activity.” ([View Highlight](https://read.readwise.io/read/01hn6e3sgq8hmb53ny0f2jng95))
- The critical aspect of this initiative is called ‘Know Your Customer,’ and it requires U.S. cloud companies to identify their foreign users rigorously. This regulation prevents entities from countries like China from accessing U.S. cloud resources for artificial intelligence development. ([View Highlight](https://read.readwise.io/read/01hn6e4dk5y5055btwy71db0y5))
- The proposed regulation imposes significant responsibilities on cloud computing firms. These companies must verify foreign customers’ identity, maintain user identification standards, and certify their compliance annually. ([View Highlight](https://read.readwise.io/read/01hn6e5hvmvc2afyftyxk62x7r))
- In October, President Joe Biden enacted an executive order mandating that developers of AI systems, which could pose threats to U.S. national [security](https://www.tomshardware.com/tag/security), economy, public health, or safety, must disclose their safety test outcomes to the U.S. government before making these systems available to the public. ([View Highlight](https://read.readwise.io/read/01hn6e667qcy17vsfw1nvzf97g))
