...
*   [Open issue](https://github.com/py-pkgs/py-pkgs/issues/new?title=Issue%20on%20page%20%2F08-ci-cd.html&body=Your%20issue%20content%20here.)

*   [.ipynb](_sources/08-ci-cd.ipynb)
*   .pdf
...
*   [8.1. An introduction to CI/CD](#an-introduction-to-ci-cd)
*   [8.2. CI/CD tools](#ci-cd-tools)
*   [8.3. Introduction to GitHub Actions](#introduction-to-github-actions)
    *   [8.3.1. Key concepts](#key-concepts)
    *   [8.3.2. A toy example](#a-toy-example)
    *   [8.3.3. Actions and commands](#actions-and-commands)
*   [8.4. Setting up continuous integration](#setting-up-continuous-integration)
    *   [8.4.1. Setup](#setup)
    *   [8.4.2. Running tests](#running-tests)
    *   [8.4.3. Recording code coverage](#recording-code-coverage)
    *   [8.4.4. Build documentation](#build-documentation)
    *   [8.4.5. Testing continuous integration](#testing-continuous-integration)
*   [8.5. Setting up continuous deployment](#setting-up-continuous-deployment)
    *   [8.5.1. Setup](#id9)
    *   [8.5.2. Automatically creating a new package version](#automatically-creating-a-new-package-version)
    *   [8.5.3. Uploading to TestPyPI and PyPI, and creating a release on GitHub](#uploading-to-testpypi-and-pypi-and-creating-a-release-on-github)
    *   [8.5.4. Testing continuous deployment](#testing-continuous-deployment)
*   [8.6. Summary](#summary)
...
span.prompt1:before{content:"\\$ "}$ mkdir \-p .github/workflows
...
changelog\_file \= "CHANGELOG.md"                       \# changelog file
build\_command \= "pip install poetry && poetry build"  \# build dists                                           
...
Finally, the last step in our CD workflow will be to create a GitHub release that corresponds to the new version published on TestPyPI and PyPI. To do this we will use a second GitHub action from PSR, [python-semantic-release/upload-to-gh-release@main](https://github.com/python-semantic-release/upload-to-gh-release). This action also requires the GitHub access token for permission to create the GitHub release. Again, we also use a conditional (see the keyword `if`) to only run this step if PSR successfully created a release earlier in the workflow.
...
We’re now ready to test out our full CI/CD workflow! Let’s go ahead and commit our new workflow file and our _`pyproject.toml`_ file (which we changed when we added the configuration options for PSR) to version control and push it to GitHub. When we commit our changes, we will be sure to include the “feat” keyword in our commit message to trigger PSR to make a minor release of our package from these changes. This will trigger our CI/CD workflow because we configured it to run when someone pushes to the “main” branch of our repository and indicates in a commit message that they have made a patch, minor or major change.
...
](/07-releasing-versioning "previous page")[

next

9\. Bibliography

](/09-bibliography "next page")
