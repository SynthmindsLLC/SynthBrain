[![](https://img.realpython.net/1c3e33fa8ce0d82a964b7f7de310e4bb)](https://srv.realpython.net/click/21871400699/?c=41831496980&p=29182759436&r=34755)

Speed[¶](#speed "Permalink to this headline")
=============================================

![https://d33wubrfki0l68.cloudfront.net/f33b6d6a18c20fd1a83677c8ea09be0a08e43231/e3c2f/_images/33175625804_e225b90f3e_k_d.jpg](https://d33wubrfki0l68.cloudfront.net/f33b6d6a18c20fd1a83677c8ea09be0a08e43231/e3c2f/_images/33175625804_e225b90f3e_k_d.jpg)
...
### The GIL[¶](#the-gil "Permalink to this headline")
...
### C Extensions[¶](#c-extensions "Permalink to this headline")

### The GIL[¶](#id1 "Permalink to this headline")
...
C Extensions[¶](#id2 "Permalink to this headline")
--------------------------------------------------

### Cython[¶](#cython "Permalink to this headline")
...
### Pyrex[¶](#pyrex "Permalink to this headline")

### Shedskin?[¶](#shedskin "Permalink to this headline")

Concurrency[¶](#concurrency "Permalink to this headline")
---------------------------------------------------------

### Concurrent.futures[¶](#concurrent-futures "Permalink to this headline")
...
def get\_webpage(url):
    page \= requests.get(url)
    return page

pool \= ThreadPoolExecutor(max\_workers\=5)

my\_urls \= \['http://google.com/'\]\*10  \# Create a list of urls

for page in pool.map(get\_webpage, my\_urls):
...
futures \= \[\]
with ProcessPoolExecutor(max\_workers\=4) as pool:
    \# Schedule the ProcessPoolExecutor to check if a number is prime
    \# and add the returned Future to our list of futures
    for p in PRIMES:
        fut \= pool.submit(is\_prime, p)
        futures.append(fut)

\# As the jobs are completed, print out the results
for number, result in as\_completed(futures):
    if result:
        print("{} is prime".format(number))
    else:
        print("{} is not prime".format(number))

The [concurrent.futures](https://docs.python.org/3/library/concurrent.futures.html) module contains two helper functions for working with Futures. The as\_completed(futures) function returns an iterator over the list of futures, yielding the futures as they complete.
...
### threading[¶](#threading "Permalink to this headline")
...
some\_thread \= Thread(get\_webpage, 'http://google.com/')
some\_thread.start()
