[![](https://img.realpython.net/6fe6464c8bf6ec2cbcf3f1f0e778156b)](https://srv.realpython.net/click/57356986228/?c=41067926597&p=29182759436&r=81074)

Code Style[¶](#code-style "Permalink to this headline")
=======================================================

![https://d33wubrfki0l68.cloudfront.net/1155a77ae015f873c4faea76d55e5ccf73a0cfa1/6c6e8/_images/33907150054_5ee79e8940_k_d.jpg](https://d33wubrfki0l68.cloudfront.net/1155a77ae015f873c4faea76d55e5ccf73a0cfa1/6c6e8/_images/33907150054_5ee79e8940_k_d.jpg)
...
General concepts[¶](#general-concepts "Permalink to this headline")
-------------------------------------------------------------------

### Explicit code[¶](#explicit-code "Permalink to this headline")

While any kind of black magic is possible with Python, the most explicit and straightforward manner is preferred.

**Bad**

def make\_complex(\*args):
    x, y \= args
    return dict(\*\*locals())

**Good**

def make\_complex(x, y):
    return {'x': x, 'y': y}
...
### One statement per line[¶](#one-statement-per-line "Permalink to this headline")
...
### Function arguments[¶](#function-arguments "Permalink to this headline")
...
The same caution as in the case of _arbitrary argument list_ is necessary, for similar reasons: these powerful techniques are to be used when there is a proven necessity to use them, and they should not be used if the simpler and clearer construct is sufficient to express the function’s intention.
...
### Avoid the magical wand[¶](#avoid-the-magical-wand "Permalink to this headline")
...
### We are all responsible users[¶](#we-are-all-responsible-users "Permalink to this headline")
...
### Returning values[¶](#returning-values "Permalink to this headline")
...
Idioms[¶](#idioms "Permalink to this headline")
-----------------------------------------------

A programming idiom, put simply, is a _way_ to write code. The notion of programming idioms is discussed amply at [c2](http://c2.com/cgi/wiki?ProgrammingIdiom) and at [Stack Overflow](https://stackoverflow.com/questions/302459/what-is-a-programming-idiom).

Idiomatic Python code is often referred to as being _Pythonic_.

Although there usually is one — and preferably only one — obvious way to do it; _the_ way to write idiomatic Python code can be non-obvious to Python beginners. So, good idioms must be consciously acquired.

Some common Python idioms follow:

### Unpacking[¶](#unpacking "Permalink to this headline")
...
for index, item in enumerate(some\_list):
...
### Create an ignored variable[¶](#create-an-ignored-variable "Permalink to this headline")

If you need to assign something (for instance, in [Unpacking](#unpacking-ref)) but will not need that variable, use `__`:
...
four\_lists \= \[\[\] for \_\_ in range(4)\]

### Create a string from a list[¶](#create-a-string-from-a-list "Permalink to this headline")

A common idiom for creating strings is to use [`str.join()`](https://docs.python.org/3/library/stdtypes.html#str.join "(in Python v3.11)") on an empty string.

letters \= \['s', 'p', 'a', 'm'\]
word \= ''.join(letters)

This will set the value of the variable _word_ to ‘spam’. This idiom can be applied to lists and tuples.

### Searching for an item in a collection[¶](#searching-for-an-item-in-a-collection "Permalink to this headline")
...
Zen of Python[¶](#zen-of-python "Permalink to this headline")
-------------------------------------------------------------
...
PEP 8[¶](#pep-8 "Permalink to this headline")
---------------------------------------------
...
optparse.py:544:21: W601 .has\_key() is deprecated, use 'in'

### Auto-Formatting[¶](#auto-formatting "Permalink to this headline")
...
### Check if a variable equals a constant[¶](#check-if-a-variable-equals-a-constant "Permalink to this headline")
...
### Access a Dictionary Element[¶](#access-a-dictionary-element "Permalink to this headline")

Don’t use the `dict.has_key()` method. Instead, use `x in d` syntax, or pass a default argument to [`dict.get()`](https://docs.python.org/3/library/stdtypes.html#dict.get "(in Python v3.11)").
...
if d.has\_key('hello'):
    ...
    print('default\_value')

**Good**:

d \= {'hello': 'world'}

print(d.get('hello', 'default\_value')) \# prints 'world'
print(d.get('thingy', 'default\_value')) \# prints 'default\_value'

\# Or:
if 'hello' in d:
    print(d\['hello'\])

### Short Ways to Manipulate Lists[¶](#short-ways-to-manipulate-lists "Permalink to this headline")
...
def make\_batches(items, batch\_size):
    """
    >>> list(make\_batches(\[1, 2, 3, 4, 5\], batch\_size=3))
    \[\[1, 2, 3\], \[4, 5\]\]
    """
    current\_batch \= \[\]
    for item in items:
        current\_batch.append(item)
        if len(current\_batch) \== batch\_size:
            yield current\_batch
            current\_batch \= \[\]
    yield current\_batch
...
### Filtering a list[¶](#filtering-a-list "Permalink to this headline")
...
filtered\_values \= \[value for value in sequence if value != x\]

\# generators don't create another list
filtered\_values \= (value for value in sequence if value != x)

#### Possible side effects of modifying the original list[¶](#possible-side-effects-of-modifying-the-original-list "Permalink to this headline")

Modifying the original list can be risky if there are other variables referencing it. But you can use _slice assignment_ if you really want to do that.

\# replace the contents of the original list
sequence\[::\] \= \[value for value in sequence if value != x\]

### Modifying the values in a list[¶](#modifying-the-values-in-a-list "Permalink to this headline")
...
Use [`enumerate()`](https://docs.python.org/3/library/functions.html#enumerate "(in Python v3.11)") keep a count of your place in the list.
...
The [`enumerate()`](https://docs.python.org/3/library/functions.html#enumerate "(in Python v3.11)") function has better readability than handling a counter manually. Moreover, it is better optimized for iterators.

### Read From a File[¶](#read-from-a-file "Permalink to this headline")
...
### Line Continuations[¶](#line-continuations "Permalink to this headline")
