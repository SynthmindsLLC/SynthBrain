[![](https://img.realpython.net/8899c4a6d15334a6b9f769e9f8bc0cf7)](https://srv.realpython.net/click/41686012779/?c=7309556696&p=29182759436&r=28453)

Logging[¶](#logging "Permalink to this headline")
=================================================

![https://d33wubrfki0l68.cloudfront.net/4a9f1d4fbeff3b43baf2d5a2becc1f2688217a3c/83efa/_images/35254379756_c9fe23f843_k_d.jpg](https://d33wubrfki0l68.cloudfront.net/4a9f1d4fbeff3b43baf2d5a2becc1f2688217a3c/83efa/_images/35254379756_c9fe23f843_k_d.jpg)

The [`logging`](https://docs.python.org/3/library/logging.html#module-logging "(in Python v3.11)") module has been a part of Python’s Standard Library since version 2.3. It is succinctly described in [**PEP 282**](https://www.python.org/dev/peps/pep-0282). The documentation is notoriously hard to read, except for the [basic logging tutorial](http://docs.python.org/3/howto/logging.html#logging-basic-tutorial).
...
… or Print?[¶](#or-print "Permalink to this headline")
------------------------------------------------------
...
*   Logging can be selectively silenced by using the method [`logging.Logger.setLevel()`](https://docs.python.org/3/library/logging.html#logging.Logger.setLevel "(in Python v3.11)") or disabled by setting the attribute `logging.Logger.disabled` to `True`.

Logging in a Library[¶](#logging-in-a-library "Permalink to this headline")
---------------------------------------------------------------------------

Notes for [configuring logging for a library](https://docs.python.org/3/howto/logging.html#configuring-logging-for-a-library) are in the [logging tutorial](http://docs.python.org/3/howto/logging.html). Because the _user_, not the library, should dictate what happens when a logging event occurs, one admonition bears repeating:

Note

It is strongly advised that you do not add any handlers other than NullHandler to your library’s loggers.

Best practice when instantiating loggers in a library is to only create them using the `__name__` global variable: the [`logging`](https://docs.python.org/3/library/logging.html#module-logging "(in Python v3.11)") module creates a hierarchy of loggers using dot notation, so using `__name__` ensures no name collisions.
...
Logging in an Application[¶](#logging-in-an-application "Permalink to this headline")
-------------------------------------------------------------------------------------
...
*   Using an INI-formatted file:
    ...
    *   **Pro**: possible to update configuration while running, using the function [`logging.config.listen()`](https://docs.python.org/3/library/logging.config.html#logging.config.listen "(in Python v3.11)") to listen on a socket.
...
*   Using a dictionary or a JSON-formatted file:
    ...
    *   **Pro**: in addition to updating while running, it is possible to load from a file using the [`json`](https://docs.python.org/3/library/json.html#module-json "(in Python v3.11)") module, in the standard library since Python 2.6.
...
### Example Configuration via an INI File[¶](#example-configuration-via-an-ini-file "Permalink to this headline")
...
fileConfig('logging\_config.ini')
logger \= logging.getLogger()
logger.debug('often makes a very good meal of %s', 'visiting tourists')

### Example Configuration via a Dictionary[¶](#example-configuration-via-a-dictionary "Permalink to this headline")
...
dictConfig(logging\_config)

logger \= logging.getLogger()
logger.debug('often makes a very good meal of %s', 'visiting tourists')

### Example Configuration Directly in Code[¶](#example-configuration-directly-in-code "Permalink to this headline")
