5\. `set` Data Structure[¶](#set-data-structure "Permalink to this headline")
=============================================================================
...
some\_list \= \['a', 'b', 'c', 'b', 'd', 'm', 'n', 'n'\]

duplicates \= \[\]
for value in some\_list:
    if some\_list.count(value) \> 1:
...
some\_list \= \['a', 'b', 'c', 'b', 'd', 'm', 'n', 'n'\]
duplicates \= set(\[x for x in some\_list if some\_list.count(x) \> 1\])
print(duplicates)
\# Output: set(\['b', 'n'\])
...
input\_set \= set(\['red', 'brown'\])
print(input\_set.intersection(valid))
\# Output: set(\['red'\])
...
valid \= set(\['yellow', 'red', 'blue', 'green', 'black'\])
input\_set \= set(\['red', 'brown'\])
print(input\_set.difference(valid))
\# Output: set(\['brown'\])

You can also create sets using the new notation:

a\_set \= {'red', 'blue', 'green'}
print(type(a\_set))
\# Output: <type 'set'>
