...
Gemini API를 사용하면 Google의 최신 생성형 AI 모델에 액세스할 수 있습니다. Gemini 튜토리얼은 원하는 프로그래밍 언어를 사용하여 Gemini를 시작하는 데 도움이 됩니다.

서버 측 애플리케이션에서 Gemini API를 호출하는 방법을 알아보세요.
...
모바일 또는 웹 앱에서 직접 Gemini API를 호출하는 방법을 알아보세요.
...
프로그래밍을 시작할 준비가 되었지만 어떤 언어를 사용해야 할지 잘 모르겠다면 [Python 가이드](https://ai.google.dev/gemini-api/docs/get-started/python?hl=ko)를 참조하세요. [Colab](https://colab.google/?hl=ko) 노트북으로 사용할 수 있으므로 최소한의 설정으로 브라우저에서 코드를 실행할 수 있습니다.
