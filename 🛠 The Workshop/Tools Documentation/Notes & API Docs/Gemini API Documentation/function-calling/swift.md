...
*   [Set up your project](#set-up-project)
    ...
    *   [Add the SDK package to your project](#add-sdk)
...
Expand to view how to store and read the API key from a `.plist` file
...
You can also review the [sample app](https://github.com/google/generative-ai-swift/tree/main/Examples/GenerativeAISample) to learn how to store your API key in a `.plist` file.

This tutorial assumes that you're accessing your API key from this on-demand resource `.plist` file.
...
3.  In the _Add Packages_ dialog, paste the package URL in the search bar:
...
Before you can make any API calls, you need to initialize the generative model. This is the basic initialization; later in this tutorial you'll update it for function calling.
...
2.  Initialize the generative model:
        ...
        // Use a model that supports function calling, like a Gemini 1.5 model// Access your API key from your on-demand resource .plist file// (see "Set up your API key" above)let model = GenerativeModel(name: "gemini-1.5-flash", apiKey: APIKey.default)
...
For demonstration purposes in this tutorial, rather than sending an actual API request, you'll be returning hardcoded values in the same format that an actual API would return.
    ...
    func makeAPIRequest(currencyFrom: String,                    currencyTo: String) -> JSONObject {  // This hypothetical API returns a JSON such as:  // {"base":"USD","rates":{"SEK": 0.091}}  return [    "base": .string(currencyFrom),    "rates": .object([currencyTo: .number(0.091)]),  ]}
...
Include as much detail as possible in the function and parameter descriptions. The generative model uses this information to determine which function to select and how to provide values for the parameters in the function call.
    ...
    let getExchangeRate = FunctionDeclaration(  name: "getExchangeRate",  description: "Get the exchange rate for currencies between countries",  parameters: [    "currencyFrom": Schema(      type: .string,      description: "The currency to convert from."    ),    "currencyTo": Schema(      type: .string,      description: "The currency to convert to."    ),  ],  requiredParameters: ["currencyFrom", "currencyTo"])
...
Specify the function declaration when initializing the generative model by setting the model's `tools` parameter:
    ...
    // Use a model that supports function calling, like a Gemini 1.5 modellet generativeModel = GenerativeModel(  name: "gemini-1.5-flash",  apiKey: apiKey,  // Specify the function declaration.  tools: [Tool(functionDeclarations: [getExchangeRate])])
...
The recommended way to use function calling is through the chat interface, since function calls fit nicely into chat's multi-turn structure.
    ...
    let chat = generativeModel.startChat()let prompt = "How much is 50 US dollars worth in Swedish krona?"// Send the message to the generative modellet response1 = try await chat.sendMessage(prompt)// Check if the model responded with a function callguard let functionCall = response1.functionCalls.first else {  fatalError("Model did not respond with a function call.")}// Print an error if the returned function was not declaredguard functionCall.name == "getExchangeRate" else {  fatalError("Unexpected function called: \(functionCall.name)")}// Verify that the names and types of the parameters match the declarationguard case let .string(currencyFrom) = functionCall.args["currencyFrom"] else {  fatalError("Missing argument: currencyFrom")}guard case let .string(currencyTo) = functionCall.args["currencyTo"] else {  fatalError("Missing argument: currencyTo")}// Call the hypothetical APIlet apiResponse = makeAPIRequest(currencyFrom: currencyFrom, currencyTo: currencyTo)// Send the API response back to the model so it can generate a text response that can be// displayed to the user.let response = try await chat.sendMessage([ModelContent(  role: "function",  parts: [.functionResponse(FunctionResponse(    name: functionCall.name,    response: apiResponse  ))])])// Log the text response.guard let modelResponse = response.text else {  fatalError("Model did not respond with text.")}print(modelResponse)
