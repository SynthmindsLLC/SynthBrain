...
> Check out our blog post [Long context prompting for Claude 2.1](https://www.anthropic.com/news/claude-2-1-prompting) to see an example of highly effective prefilling.
...
2.  **Control output format**: Prefilling allows you to specify the exact format you want Claude to use for its output. This is especially handy when working with structured data formats like JSON or XML. For more details on this, see our guide on [controlling output format](/en/docs/control-output-format).
3.  **Maintain character consistency**: In role-play scenarios, prefilling Claude’s response can help Claude stay in character throughout a long conversation. By consistently reminding Claude of its role in the `Assistant` message, you can better ensure that Claude maintains the desired persona. Check out [keep Claude stay in character](/en/docs/keep-claude-in-character) for more details.
...
| Role | Good Prompt |
| --- | --- |
| User | Please extract the name, size, price, and color from this product description and output it within a JSON object.  
...
[Let Claude think](/en/docs/let-claude-think)[Control output format (JSON mode)](/en/docs/control-output-format)

[x](https://x.com/AnthropicAI)[linkedin](https://www.linkedin.com/company/anthropicresearch)

*   [Why prefill Claude’s response?](#why-prefill-claudes-response)
