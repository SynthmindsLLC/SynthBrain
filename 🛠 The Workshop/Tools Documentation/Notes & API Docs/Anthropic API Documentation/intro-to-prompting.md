...
Claude is a conversational assistant, based on a large language model ([LLM](/en/docs/glossary#llm)). The model uses all the text that you have sent it (your prompt) and all the text it has generated so far to predict the next [token](/en/docs/glossary#tokens) that would be most helpful.
...
Claude can also only see (and make predictions on) what is in its [context window](/en/docs/glossary#context-window). It can’t remember previous conversations unless you put them in the prompt, and it can’t open links.

> If you’re ready to chat with Claude, visit [claude.ai](https://claude.ai/) to chat with Claude on the web or check out our [quickstart guide](/en/docs/quickstart-guide) to get quickly up and running using the API.
...
The maximum prompt length that Claude can see is its [context window](/en/docs/glossary#context-window). See our [model comparison](/en/docs/models-overview#model-comparison) table for a list of context window sizes per model.
...
Our [prompt library](/en/prompt-library) features an extensive collection of prompts for various use cases, covering both work and play. It’s a great resource to explore when you’re looking for ideas or want to understand how to use Claude to solve specific problems. From creative writing to data analysis to character role-play, the prompt library has something for everyone.

> **Note**: The [prompt library](/en/prompt-library) currently features only prompts without images. Prompts involving images are coming soon.
...
We also provide an experimental [helper metaprompt](/en/docs/helper-metaprompt-experimental) that prompts Claude to create a prompt for you, based on guidelines you provide. The metaprompt is experimental, but may be helpful for drafting an initial prompt or quickly creating many prompt variations.

Happy prompting, and don’t forget to share your creations in our [Discord community](https://www.anthropic.com/discord)!

[Getting access to Claude](/en/docs/getting-access-to-claude)[Migrating from other models](/en/docs/migrating-from-other-models)

[x](https://x.com/AnthropicAI)[linkedin](https://www.linkedin.com/company/anthropicresearch)

*   [What is a prompt?](#what-is-a-prompt)
*   [Claude works by sequence prediction](#claude-works-by-sequence-prediction)
