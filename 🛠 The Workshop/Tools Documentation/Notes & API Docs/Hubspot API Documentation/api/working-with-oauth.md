...
OAuth is a secure means of authentication that uses authorization tokens rather than a password to connect your app to a user account. Initiating OAuth access is the first step towards allowing users to [install your app](https://developers.hubspot.com/docs/faq/installing-an-app) in their HubSpot accounts.

**Please note:**

*   any app designed for installation by multiple HubSpot accounts or listing on the App Marketplace must use OAuth. 
*   users installing apps in their HubSpot account must either be a [super admin](https://knowledge.hubspot.com/settings/hubspot-user-permissions-guide#super-admin) or have [App Marketplace Access](https://knowledge.hubspot.com/settings/hubspot-user-permissions-guide#settings) permissions.

##### Recommended resources

*   The [OAuth Quickstart Guide](https://developers.hubspot.com/docs-beta/oauth-quickstart-guide) will get you up and running with a working example app.
*   This [HubSpot Academy tutorial](https://app.hubspot.com/l/academy/tracks/71/593/2967?_ga=2.93814047.341006870.1586180142-500942594.1573763828) provides a quick introduction on using OAuth with HubSpot, including a breakdown of the HubSpot-OAuth flow and how to refresh an access token.

Initiating an integration with OAuth 2.0[](https://developers.hubspot.com/docs/api/working-with-oauth#initiating-an-integration-with-oauth-2-0)
-----------------------------------------------------------------------------------------------------------------------------------------------

To initiate an integration with OAuth 2.0:

*   First, [create an app](/docs-beta/creating-an-app) in a [HubSpot developer account](https://app.hubspot.com/signup/developers/?_ga=2.263608837.869402458.1577115763-500942594.1573763828). After creating the app, you'll be able to find  the app's client ID and client secret on the Auth page of your app settings.

![MyHubSpotApp](https://developers.hubspot.com/hs-fs/hubfs/MyHubSpotApp.png?width=1293&name=MyHubSpotApp.png)
...
*   Send users installing your app to the authorization URL, where they'll be presented with a screen that allows them to select their account and grant access to your integration. After granting access, they'll be redirected back to your application via a `redirect_url`, which will have a code query parameter appended to it. You'll use that code and the client secret to get an [access\_token and refresh\_token](/docs-beta/oauth/tokens) from HubSpot.
...
| State | `&state=y`  
If this parameter is included in the authorization URL, the value will be included in a state query parameter when the user is directed to the redirect\_url. | A string value that can be used to maintain the user's state when they're redirected back to your app. |
...
![granular-scopes-selection](https://developers.hubspot.com/hs-fs/hubfs/Knowledge_Base_2021/Developer/granular-scopes-selection.png?width=746&name=granular-scopes-selection.png)
...
Related docs[](https://developers.hubspot.com/docs/api/working-with-oauth#related-docs)
---------------------------------------------------------------------------------------
...
[Managing tokens](/docs/api/oauth/tokens)

* * *

Share your feedback[](https://developers.hubspot.com/docs/api/working-with-oauth#page-feedback)
-----------------------------------------------------------------------------------------------
