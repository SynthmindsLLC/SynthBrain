...
*   Review the [key concepts of CMS Hub](/docs/cms/key-concepts), even if you've been building on HubSpot for a long time. You will gain a better understanding of how all the pieces fit.
*   You will also need to use the [CMS CLI](/docs/cms/developer-reference/local-development-cli), so if you don't have it installed, [go set it up](/docs/cms/guides/getting-started-with-local-development).
...
Theme[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#theme)
--------------------------------------------------------------------------------------------------

CMS Hub [themes](/docs/cms/building-blocks/themes) are a package of templates, modules, CSS, JSON, and JS files. Themes being a package has meaning throughout the content editor experience. 
...
### 1\. Place existing files in a containing folder[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#place-existing-files-in-a-containing-folder)
...
### 2\. Make containing folder a theme[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#make-containing-folder-a-theme)
...
JSON files are special and can't currently be created in the design manager. These files CAN be uploaded through the [CMS CLI](/docs/developer-reference/local-development-cms-cli). 
...
3.  Copy the [example `theme.json` file on the themes doc](/docs/cms/building-blocks/themes#theme-json).
...
### 3\. Add theme fields[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#add-theme-fields)
...
![Screenshot of global content and themes setting in user permissions](https://327485.fs1.hubspotusercontent-na1.net/hubfs/327485/global%20content%20theme%20permissions.png "Screenshot of global content and themes setting in user permissions")

Clone design manager drag and drop as HTML[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#clone-design-manager-drag-and-drop-as-html)
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------

[Drag and drop areas](/docs/cms/building-blocks/templates/drag-and-drop-areas), and [global partials](/docs/cms/building-blocks/global-content) require the use of [coded HTML + HubL](/docs/cms/building-blocks/templates/html-hubl-templates) files. If your existing website may be built using the old drag and drop design manager system, and you want to use those new features, those templates may need to be cloned as HTML.
...
Drag and drop areas[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#drag-and-drop-areas)
------------------------------------------------------------------------------------------------------------------------------

[Drag and drop areas](/docs/cms/building-blocks/templates/drag-and-drop-areas) can be added to templates to provide content creators a way to place modules both horizontally and vertically on a page. Drag and drop areas also provide additional styling control for the content creator. Because a template with a drag and drop area can be used to create many different layouts, this frees up the developer to focus on the technical aspects of site creation and updating.

Drag and drop areas are a new feature and are not the same as [Design Manager drag and drop templates](/docs/cms/building-blocks/templates/drag-and-drop-templates). They are also not supported in Design Manager drag and drop templates. To add them to templates previously built using the drag and drop template builder see clone design manager drag and drop as HTML.

#### Does it make sense to convert every existing page to use drag and drop areas?[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#does-it-make-sense-to-convert-every-existing-page-to-use-drag-and-drop-areas-)
...
### Converting existing templates[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#converting-existing-templates)
...
1.  The easiest solution is to find the instances of flexible columns, which use the HubL tag [`{% widget_container "my_unique_column_name" %}`](/docs/cms/hubl/tags#flexible-column).
2.  Replace each instance as necessary with [`{% dnd_area "my_unique_area_name" %}`](/docs/cms/hubl/tags/dnd-areas).
...
### How do I migrate existing pages?[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#how-do-i-migrate-existing-pages-)
...
If you want to convert a page to using drag and drop areas the safest way to do it, is to use [content staging](/docs/cms/features/content-staging).
...
Global partials[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#global-partials)
----------------------------------------------------------------------------------------------------------------------

[Global Partials](/docs/building-blocks/global-content) are pieces of HTML & HubL content that can be reused across your entire website. The most common types of global partials are website headers, sidebars, and footers. For content creators global partials manifest themselves in the content editor as regions they can select to edit in the global content editor. The global content editor looks just like the page editor but is used to edit global content. 
...
![global content & theme setting in user permissions](https://327485.fs1.hubspotusercontent-na1.net/hubfs/327485/global%20content%20theme%20permissions.png "global content & theme setting in user permissions")

Code Alerts[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#code-alerts)
--------------------------------------------------------------------------------------------------------------

For accounts on CMS Hub Enterprise, [Code Alerts](/docs/cms/developer-reference/debugging-and-errors/code-alerts) provides a centralized location for developers and IT managers to see an overview of issues that affect performance and rendering of pages and assets on the account.
...
Share your feedback[](https://developers.hubspot.com/docs/cms/guides/add-theme-features-to-existing-sites#page-feedback)
------------------------------------------------------------------------------------------------------------------------
