---
description: 
created date: '[[2024-06-17]]'
type: 
excalidraw-plugin: parsed
tags:
  - excalidraw
excalidraw-open-md: true
relationships:
file folder: _📭 Inbox
---
```record
```
# [[Synthminds Education Roadmap]]

# Education

## Major Projects
### Creating Advanced Courses

**Team**:
- Tomer - Manage process
- Israel - QA
- Goda  - recording videos
- Tanner - Data Analysis
- Josh - design course
- Sjoerd - automation course

**Course List** Release Dates
- Advanced Prompt Engineering - 7/24
	- Joe
	- Uros
- Advanced Design - 8/26
	- Josh
- Advanced Data Analytics - 9/23
	- Tanner
	- Will
- Automation Engineering - 10/21
	- Sjoerd

**Tasks**:
- APE
	- move to Kajabi
	- Record videos
	- QA
- Design
	- pull out and expand advanced aspects, and revise
	- move to Kajabi
	- Record videos
	- QA
- Data
	- pull out and expand advanced aspects
	- move to kajabi
	- record/edit videos
	- QA
- Automation
	- write course - provide outline, notes, documentation, research
	- edit course
	- record videos
	- move to kajabi
	- QA

### Organizing Kajabi

**Team**:
- Tomer
- Josh

**Tasks**
- creating course pages
- designing courses
- 

### Creating payment/services structure

**Team**:
- Josh
- Goda
- Bradford

### Marketing

**Team**:
- Goda
- Josh
- Intern

### Sales

**Team**:
- Wes
- Bradford
- Amy

# Internal

# Excalidraw Data
## Text Elements
%%
## Drawing
```compressed-json

```
%%